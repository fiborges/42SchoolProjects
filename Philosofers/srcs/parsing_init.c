...
int	ft_atoi_adapt(const char *str)
{
	int		i;
	long	sum;
...
int	parse_args(int argc, char **argv, t_p *p)
{
	...
	{
		p->a.total = ft_atoi_adapt(argv[1]);
		p->a.die = ft_atoi_adapt(argv[2]);
		p->a.eat = ft_atoi_adapt(argv[3]);
		p->a.sleep = ft_atoi_adapt(argv[4]);
		p->a.m_eat = -1;
		if (argc == 6)
			p->a.m_eat = ft_atoi_adapt(argv[5]);
