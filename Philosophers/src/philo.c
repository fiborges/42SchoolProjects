...
static int	initialize_philo(t_philo *philo)
{
	int	i;
...
static int	setup_simulation(t_philo *philo, int n, char *args[])
{
	philo->start = now();
...
static void	set_thread(t_philo *philo)
{
	...
	while (i < philo->nb)
	{
		...
		idx[i]->index = i;
		idx[i]->philo = philo;
		pthread_create(&philosophers[i], NULL, ft_state, idx[i]);
		i++;
	}
	while (philo->sim)
		;
...
static void	free_philo_memory(t_philo *philo)
{
	int	i;
...
e] [time_to_eat] [time_to_sleep] [nb_of_times_each_philosopher_must_eat]"));
	...
	if (setup_simulation(philo, argc, argv) >= 0)
		set_thread(philo);
	free_philo_memory(philo);
	free(philo);
}
